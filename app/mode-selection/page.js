...
export default function ModeSelection() {
  ...
  const { movies, setMaxComparisons, setRankingMode } = useMovieStore();
  ...
  const handleModeSelect = (mode, comparisonsCount) => {
    setRankingMode(mode);
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-3 lg:gap-8 max-w-md lg:max-w-4xl mx-auto place-items-center">
        {modeOptions.map(mode => (
          <ModeOption
            key={mode.id}
            {...mode}
            onClick={() => handleModeSelect(mode.id, mode.comparisons)}
          />
        ))}
      </div>
